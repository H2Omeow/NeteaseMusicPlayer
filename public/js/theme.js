...
function updateThemeBlurOnly(val) {
...
function initBg() {
  ...
  updateBgStatus('正在读取本地壁纸目录...', false);
  ...
  .catch(function(e) {
    ...
    updateBgStatus('本地加载失败，已使用随机兜底壁纸', true);
    setBg(fallbackUrl);
  });
}

function selectAndSetBg(files, isMobile) {
  ...
  updateBgStatus('正在并行检测并计算 ' + sampleSize + ' 张壁纸的画面契合度...', false);
  ...
  function applyBestFit() {
    if (results.length > 0) {
      ...
      var best = results[0];
      setBg(best.url);
      updateBgStatus('✅ 已载入屏幕契合度最高的壁纸: ' + best.name + ' (' + best.w + 'x' + best.h + ', 画面偏离: ' + ((best.fitFactor - 1) * 100).toFixed(0) + '%)', false);
    } else {
      updateBgStatus('❌ 候选壁纸解析失败，已启用图床兜底', true);
      var fallbackUrl = 'https://picsum.photos/' + (isMobile ? '1080/1920' : '1920/1080') + '?random=1';
      setBg(fallbackUrl);
    }
  }
...
function updateBgStatus(text, isError) {
...
function changeMaskOpacity(val) {
